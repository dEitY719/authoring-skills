...
Both issues are created by **reusing `gh-issue:issue-create`** — never by calling
...
**Prerequisite.** `gh-issue:issue-create` ships in the sibling marketplace
`dEitY719/gh-issue-skills` (plugin `gh-issue`), not in this repo. Confirm it
resolves before Step 6 — `Skill(gh-issue:issue-create, ...)` and the refactor
...
Invoke `Skill(gh-issue:issue-create, "<remote>")`, having first laid out the design
in the conversation so gh-issue:issue-create's classifier lands on `refactor`:
...
`Skill(gh-issue:issue-create, "<remote>")` with explicit `docs` intent (문서 자체
...
— everything else goes through `gh-issue:issue-create`, which binds its own host.
