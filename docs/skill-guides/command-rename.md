...
`/gh-flow:issue <refactor-issue-number>` 가 수행합니다.
...
| 하고 싶은 일 | 담당 | 비고 |
|--------------|------|------|
| 실제로 파일/심볼을 리네임하고 커밋·PR 까지 | `/gh-flow:issue <N>` | command-rename 이 만든 refactor 이슈 번호를 넘긴다 |
...
별도의 `/gh-flow:issue` 실행입니다. SKILL.md 의 Role 절과
...
2. **발견(discovery)** (`references/discovery.md`) — 정의 지점
   ...
   이슈의 "범위(Scope)" 가 됩니다.
3. **SSOT 대조 + 규칙 공백 탐지** (`references/ssot-check.md`) — 세 SSOT 문서는
   `dEitY719/dotfiles` 체크아웃(`$DOTFILES_ROOT`, 기본 `$HOME/dotfiles`) 기준입니다.
...
7. **리포트** (`references/report-template.md`) — 생성된 이슈 번호 + URL,
   `[OK]`/`[FAIL]` 판정, 그리고 `Next: /gh-flow:issue <refactor-issue-number>` 힌트.
...
- **소스는 절대 건드리지 않습니다.** 편집도 커밋도 없고, 모든 탐색은
  `grep`/`Read` 읽기 전용입니다. 리네임은 `/gh-flow:issue` 의 몫입니다.
